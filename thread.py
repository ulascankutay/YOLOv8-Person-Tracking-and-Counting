...
classNames = ["person"]
mask = cv2.imread("/home/ulascan/Desktop/Nesne Tanıma/Maske/maske4.jpg")

# Tracking
tracker = Sort(max_age=30, min_hits=3, iou_threshold=0.5)

cap = cv2.VideoCapture("/home/ulascan/Desktop/Nesne Tanıma/Videolar/deneme2.webm")
...
del size[2]
size.reverse()
video = cv2.VideoWriter("SONUC3.mp4", cv2_fourcc, 60, size) #output video name, fourcc, fps, size

model = YOLO("yolov8n.pt")

# Çizgi sınırları (giriş ve çıkış tespiti için)
limits = [4, 180, 590, 164]
...
def main():
    ...
    while cap.isOpened():
        ...
        if not result_queue.empty():
            ...
            cv2.putText(frame, f'Giris: {len(enteredIDs)}', (30, 50), cv2.FONT_HERSHEY_PLAIN, 3, (50, 50, 255), 5)
            cv2.putText(frame, f'Cikis: {len(exitedIDs)}', (30, 125), cv2.FONT_HERSHEY_PLAIN, 3, (50, 50, 255), 5)
